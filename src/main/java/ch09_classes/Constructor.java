...
/*
    ...
    특징 :
        1. 클래스의 이름과 생성자 이름은 동일하다.
            (다른 메서드값들과 달리 대문자로 시작한다)
        2. return값이 없다. -> call(), call2() 유형으로 볼 수 있다.
        3. 객체 초기화 : 생성자는 객체의 필드(속성) 초기화를 담당한다.

    생성자의 정의 방식
        1. 기본 생성자 : 매개 변수가 없는 생성자
            (사실 클래스를 만들 대 default로 생성되어 정의하지 않을 때도 있음
...
public class Constructor {
    ...
    //기본 생성자 정의 -> 직접하면 call() 유형과 같은 형태 [x | x]
    Constructor() {
        System.out.println("NoArgsConstructor(기본생성자)");
    }

    //매개변수 생성자 -> 기본적으로 만들어지지 않습니다. 개발자가 정의하여야 함.
    //근데 그 경우에도 기본 생성자와 매개변수 생성자를 둘 다 쓰고 싶다면
    //기본 생성자도 정의해야 함.
    Constructor(int number) {
        System.out.println("RequiredArgsConstructor(int 매개변수를 필수로 요구하는 생성자)");
        this.num = number; //this : 해당 클래스에서 객체를 만들게 되면 객체 이름으로 대체됨(임시적인 객체명)
    }
    ...
    Constructor(int num, String name) {
        this.num = num;
        this.name= name;
        System.out.println("AllArgsConstructor(int 매개변수를 전체로 요구하는 생성자)");
    }
