...
public class CentralControl {
    ...
    public void performSpecificMethod() {
        for (Power device : deviceArray) {
            if(device instanceof Tv) {  //얘가 실행된다면 배열 내부의 element가 Tv 클래스로 생서되었다가 Power
                Tv tv = (Tv)device; //다운캐스팅은 명시적이여야하기 때문에
                tv.changeChannel();
